{{-- Modal penilaian 1 kamar (dipakai halaman Inspeksi Hari Ini, sekarang per SESI).
     $kamar = model kamar, $sesi = 'pagi'|'sore', $nilai = AsramaPenilaianKamar|null (untuk koreksi) --}}
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto" @click.away="openModal = false">
        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50/60 flex items-center justify-between">
            <div>
                ...
                <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400 mt-0.5">{{ \App\Http\Controllers\AsramaPenilaianController::labelSesi($sesi) }} · {{ \App\Http\Controllers\AsramaPenilaianController::jendelaSesi($sesi) }}</p>
            </div>
        ...
        <form action="{{ route('asrama.penilaian.simpanSkor', $kamar->id) }}" method="POST" class="p-5 sm:p-6 space-y-5">
            ...
            @foreach($kriterias as $key => $judul)
                @php $skorLama = $nilai ? (int) $nilai->{'skor_' . $key} : 0; @endphp
                <div>
                    <div class="text-[13px] font-bold text-slate-700 mb-2">{{ $key }}. {{ $judul }}</div>
                    <div class="grid grid-cols-5 gap-2">
                        @for($i=1; $i<=5; $i++)
                        <label for="k{{$key}}_{{$sesi}}_{{$kamar->id}}_{{$i}}" class="cursor-pointer">
                            <input type="radio" id="k{{$key}}_{{$sesi}}_{{$kamar->id}}_{{$i}}" name="skor_{{$key}}" value="{{$i}}" class="peer sr-only" {{ $skorLama === $i ? 'checked' : '' }} required>
            ...
            <div>
                ...
                <textarea name="catatan" rows="2" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-700 placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition" placeholder="Catatan opsional...">{{ $nilai->catatan ?? '' }}</textarea>
            </div>
