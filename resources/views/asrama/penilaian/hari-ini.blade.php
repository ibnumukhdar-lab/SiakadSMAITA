<x-app-layout>
    @php
        ...
        $warnaDivisi = $divisi === 'putra' ? 'putra' : 'putri';
    @endphp
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-5">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-0.5">
                        Divisi {{ ucfirst($divisi) }} · dua sesi sehari · {{ \Carbon\Carbon::parse($tanggal)->translatedFormat('d F Y') }}
                    </p>
                </div>
            ...
            {{-- Pilih divisi: halaman putra & putri dipisah --}}
            <div class="grid grid-cols-2 gap-3 mb-6">
                <a href="{{ route('asrama.penilaian.hariIni', ['divisi' => 'putra']) }}"
                   class="flex flex-col items-center justify-center gap-0.5 py-3 rounded-2xl font-extrabold text-[15px] border transition {{ $divisi === 'putra' ? 'bg-blue-900 text-white border-blue-900 shadow-sm' : 'bg-white text-slate-500 border-slate-200 hover:bg-blue-50 hover:text-blue-700' }}">
                    👦 Divisi Putra
                    <span class="text-[11px] font-bold {{ $divisi === 'putra' ? 'text-blue-100' : 'text-slate-400' }}">
                        {{ $progres['putra']['jumlah'] }} kamar · Pagi {{ $progres['putra']['pagi'] }}/{{ $progres['putra']['jumlah'] }} · Sore {{ $progres['putra']['sore'] }}/{{ $progres['putra']['jumlah'] }}
                    </span>
                </a>
                <a href="{{ route('asrama.penilaian.hariIni', ['divisi' => 'putri']) }}"
                   class="flex flex-col items-center justify-center gap-0.5 py-3 rounded-2xl font-extrabold text-[15px] border transition {{ $divisi === 'putri' ? 'bg-rose-500 text-white border-rose-500 shadow-sm' : 'bg-white text-slate-500 border-slate-200 hover:bg-rose-50 hover:text-rose-600' }}">
                    👧 Divisi Putri
                    <span class="text-[11px] font-bold {{ $divisi === 'putri' ? 'text-rose-100' : 'text-slate-400' }}">
                        {{ $progres['putri']['jumlah'] }} kamar · Pagi {{ $progres['putri']['pagi'] }}/{{ $progres['putri']['jumlah'] }} · Sore {{ $progres['putri']['sore'] }}/{{ $progres['putri']['jumlah'] }}
                    </span>
                </a>
            </div>
            ...
            @foreach(\App\Http\Controllers\AsramaPenilaianController::SESI as $sesi => $info)
                ...
                <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden mb-5">
                    <div class="px-5 py-4 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h4 class="text-[15px] font-extrabold text-slate-800">{{ $info['ikon'] }} {{ $info['label'] }} — {{ ucfirst($divisi) }}</h4>
                    ...
                    <div class="p-5 sm:p-6">
                        ...
                        @elseif($data['draft']->status === 'final')
                            <div class="bg-emerald-50 border border-emerald-200 text-emerald-700 text-[13px] font-bold rounded-xl px-4 py-3">
                                ...
                                <a href="{{ route('asrama.penilaian.index') }}" class="text-blue-900 underline ms-1">Lihat di histori</a>
                            </div>
                        @else
                            <div class="flex flex-wrap items-center justify-between gap-3 mb-2">
                                <div class="text-[12px] font-bold text-slate-600">{{ $sudahDinilai }} / {{ $jumlahKamar }} kamar dinilai</div>
                                @if($sudahDinilai > 0)
                                    <form action="{{ route('asrama.penilaian.reset') }}" method="POST" class="m-0" onsubmit="return confirm('Reset penilaian {{ $info['label'] }} divisi {{ ucfirst($divisi) }}? Skor yang sudah diisi akan dihapus.');">
                                        @csrf
                            ...
                            @if($data['selesai'])
                                <div class="mb-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-center">
                                    <h5 class="text-[13px] font-extrabold text-emerald-800 mb-1">🪄 Seluruh kamar {{ ucfirst($divisi) }} sudah dinilai pada {{ $info['label'] }}</h5>
                            ...
                            <div class="space-y-3">
                                @foreach($data['kamar'] as $kamar)
                                    ...
                                    <div x-data="{ openModal: false }">
                                        ...
                                        @if($bolehIsi)
                                            @include('asrama.penilaian._form_modal', ['sesi' => $sesi, 'nilai' => $nilai, 'kamar' => $kamar])
                                        @endif
                                    </div>
                                @endforeach
                            </div>
